...
def capacity_based_producer_side_metrics(data, min_under_demand, nodelist , topk, envy_cap,  topk_list, new_capacity):
    ...
    for i in range(len(counts)-1) : # , c in enumerate(counts):
        poi = id[i] + 1 # Add 1 for making id mapping

        c = counts[i]
        if c >= new_capacity[poi]:
            CFSP = CFSP + 1.0

        iPE_value = iPE_value + c / (m * k * new_capacity[poi] + 1e-8) * math.log(c / (m * k * new_capacity[poi] + 1e-8), n)
        eLP_value = eLP_value + max((counts_before[i] - c) / counts_before[i], 0.0)
        un_Demand_ratio[poi] = max((new_capacity[poi] - c)/new_capacity[poi], 0.0)



    CFSP = CFSP / len(counts)
    iPE_value = -iPE_value
    eLP_value = eLP_value / len(counts)

    CEFA = capculate_Envy_Free_Score(un_Demand_ratio, envy_cap ) # , min_under_demand)

    return CFSP, iPE_value, eLP_value, CEFA
...
def CAFPR_Model(V, topk, new_capacity, min_capacity):
    ...
    CAFPR_A = np.asarray([[i for i in Recommended_POIs_to_User[u]] for u in Recommended_POIs_to_User.keys()])

    CAFPR_A = np.asarray([[i for i in y] for y in  CAFPR_A])
...
def main(dataset,dataFrame):
    ...
    dfNodes = pd.read_excel('costProfCat-' + dataset + '-all.xlsx')
    ...
    for iteration in range(iteration_number):
        ...
        with tf.Graph().as_default(), tf.compat.v1.Session() as session:
            ...
            print("Minimum capacity ", minimum_capacity)

            CAFPR_A = CAFPR_Model(list_value, config.topk, minimum_capacity, minimum_capacity)
            ...
            flat_CAFPR_A = CAFPR_A.reshape(CAFPR_A.shape[0] * topk).T

            gini_CAFPR = gini(flat_CAFPR_A)
            ...
            CAFPR_results_P_R_F = evalutation_metrics(CAFPR_A, new_test_set_sequence_y, config.topk)
            # print("CAFPR P-R-F = ",CAFPR_results_P_R_F)
            ...
            Fair_CFSP, Fair_IpS, Fair_ElP, Fair_CEFA = capacity_based_producer_side_metrics(Fair_A,
            ...
            Fair_CFSP_Plus, Fair_IpS_Plus, Fair_ElP_Plus, Fair_CEFA_Plus = capacity_based_producer_side_metrics(Fair_A_Plus, config.min_under_demand,nodelist, topk, config.envy_cap,TOPK_A,minimum_capacity)
            ...
            CAFPR_CFSP, CAFPR_IpS, CAFPR_ElP, CAFPR_CEFA = capacity_based_producer_side_metrics(CAFPR_A, config.min_under_demand,nodelist, topk, config.envy_cap,TOPK_A,minimum_capacity)

            # results_after.insert(i for i in [H_value, Z_value,L_value,Y_value, Loss_value,Std_value])
            CAFPR_all_results = CAFPR_results_P_R_F + [CAFPR_CFSP, CAFPR_IpS,CAFPR_CEFA, CAFPR_ElP] + [gini_CAFPR]
            Fiar_all_results = Fair_results_P_R_F + [Fair_CFSP, Fair_IpS,
                                                     Fair_CEFA, Fair_ElP] + [gini_Fair]
            Fiar_all_results_Plus = Fair_results_P_R_F_Plus + [Fair_CFSP_Plus, Fair_IpS_Plus,
                                                     Fair_CEFA_Plus, Fair_ElP_Plus] + [gini_Fair_Plus]

            TOPK_all_results = TOPK_results_P_R_F + [TOPK_FSP, TOPK_IpS,
                                                     TOPK_Envy_Free_Score, TOPK_ElP] + [gini_TOPK]
            Low_TOPK_all_results = Low_TOPK_results_P_R_F + [ Low_TOPK_FSP,
                                                             Low_TOPK_IpS, Low_TOPK_Envy_Free_Score, Low_TOPK_ElP] +  [gini_LOWK]
            Random_TOPK_all_results = Random_TOPK_results_P_R_F + [ Random_TOPK_FSP, Random_TOPK_IpS,
                                                                   Random_TOPK_Envy_Free_Score, Random_TOPK_ElP] +  [gini_Random]
            Mixed_TOPK_all_results = Mixed_TOPK_results_P_R_F + [ Mixed_TOPK_FSP, Mixed_TOPK_IpS,
                                                                 Mixed_TOPK_Envy_Free_Score, Mixed_TOPK_ElP] + [gini_Mixed]


            dataFrame.at[dataFrame.shape[0]] = ["FTOPK", dataset, topk] + CAFPR_all_results
            ...
            data_results_cafpr.append(CAFPR_all_results)
    ...
    #
    data_results_cafpr = np.asarray(data_results_cafpr)
    data_results_cafpr_mean = [sum(i) / len(data_results_rand) for i in zip(*data_results_cafpr)]
    ...
    print(dataset, " Final Results CAFPR= ", data_results_cafpr_mean)
...
if __name__ == "__main__":
    ...
    for topk in [5,10]:
        ...
        config.topk = topk
        columnName = ["Algo", "dataset", "top-k", "precision", "recall", "F1-score", "ndcg", "CFSP", "iPE_value", "CEFA", "eLP_value", "gini_index"]
        ...
        dataFrame.to_excel("CAFPR_ASC" + str(config.topk) + "_" + str(Config.min_under_demand) +"_"+str(Config.envy_cap)+ ".xlsx", index=False)
